function love.update(dt) 
	...
	if game_state == 'show_level_title' then
		...
		level_title_countdown = level_title_countdown - 1	

		if level_title_countdown == 0 then

			game_state = 'level_title_scroll_out'
		end
		return
	
	elseif game_state == 'level_title_scroll_out' then

		level_title_scroll = level_title_scroll + 10

		if level_title_scroll == 250 then
	...
	if not (end_game) then
		...
		if not draining_cylinders then
			...
			if color_level.red == color_level.max
			and color_level.blue == color_level.max
			and color_level.green == color_level.max
			and color_level.yellow == color_level.max then
				draining_cylinders = true
				cylinder_drain_countdown = 64
				color_level.red = 0
				color_level.blue = 0
				color_level.green = 0
				color_level.yellow = 0
				sfx.drain:play()
			end
		end

		if draining_cylinders then

			cylinder_drain_countdown = cylinder_drain_countdown - 1

			if cylinder_drain_countdown == 0 then
				draining_cylinders = false
				color_level.max = color_level.max + 1
			end
		end
		...
		for i=1, #topcolumn do
			for ii=1, #topcolumn[i] do
				topcolumn[i][ii].angle = topcolumn[i][ii].angle + 4
			end
		end

		for i=1, #leftrow do
			for ii=1, #leftrow[i] do
				leftrow[i][ii].angle = leftrow[i][ii].angle + 4
			end
		end

		for i=1, #bottomcolumn do
			for ii=1, #bottomcolumn[i] do
				bottomcolumn[i][ii].angle = bottomcolumn[i][ii].angle + 4
			end
		end

		for i=1, #rightrow do
			for ii=1, #rightrow[i] do
				rightrow[i][ii].angle = rightrow[i][ii].angle + 4
			end
		end
							...
							if topcolumn[i][ii].grid_x == triangle.grid_x then
								...
								if topcolumn[i][ii].grid_y == triangle.grid_y then
									...
									if triangle.color ~= topcolumn[i][ii].color then
										...
										if not draining_cylinders then
											local _color = topcolumn[i][ii].color
											if color_level[_color] < color_level.max then
												color_level[_color] = color_level[_color] + 1
											end
										end
							...
							if leftrow[i][ii].grid_y == triangle.grid_y then
								...
								if leftrow[i][ii].grid_x == triangle.grid_x then
									...
									if leftrow[i][ii].color ~= triangle.color then
										...
										if not draining_cylinders then
											local _color = leftrow[i][ii].color
											if color_level[_color] < color_level.max then
												color_level[_color] = color_level[_color] + 1
											end
										end
							...
							if bottomcolumn[i][ii].grid_x == triangle.grid_x then
								...
								if bottomcolumn[i][ii].grid_y == triangle.grid_y then
									...
									if bottomcolumn[i][ii].color ~= triangle.color then
										...
										if not draining_cylinders then
											local _color = bottomcolumn[i][ii].color
											if color_level[_color] < color_level.max then
												color_level[_color] = color_level[_color] + 1
											end
										end
							...
							if rightrow[i][ii].grid_y == triangle.grid_y then
								...
								if rightrow[i][ii].grid_x == triangle.grid_x then
									...
									if rightrow[i][ii].color ~= triangle.color then
										...
										if not draining_cylinders then
											local _color = rightrow[i][ii].color
											if color_level[_color] < color_level.max then
												color_level[_color] = color_level[_color] + 1
											end
										end
